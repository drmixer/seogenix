...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!competitorSiteId || !url) {
      return new Response(
        JSON.stringify({ error: "Missing required parameters: competitorSiteId and url" }),
        { 
          status: 400, 
    ...
    try {
      const response = await fetch(url, {
        headers: {
          'User-Agent': 'SEOgenix-Bot/1.0 (Competitor Analyzer)'
        }
      })
      ...
      console.error('Error fetching competitor website:', error)
      websiteContent = 'Unable to fetch competitor website content'
    }

    const prompt = `
Analyze this competitor website for AI visibility and provide scores from 0-100 for each category:

Competitor Website URL: ${url}
...
`
    ...
    try {
      ...
      // Fallback scores for competitor
      scores = {
        ai_visibility_score: Math.floor(Math.random() * 40) + 45,
        schema_score: Math.floor(Math.random() * 40) + 35,
        semantic_score: Math.floor(Math.random() * 40) + 55,
        citation_score: Math.floor(Math.random() * 40) + 40,
        technical_seo_score: Math.floor(Math.random() * 40) + 50
      }
    }

    // Create competitor audit record
    const audit = {
    ...
    console.error('Error in analyzeCompetitorSite function:', error)
    return new Response(
